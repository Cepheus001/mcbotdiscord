    ...
    const token = 'OTYzNTIzNjQ1ODQ0NjMxNjI0.GDWkfa.QYj9WtN9qARugghhj8gAMYKmIQDIEfFMng9MCw';
    ...
    bot.on('ready', () => {
        ...
        setInterval(() => {
            try {
                util.status('IntertwinedGaming.com', 25565).then((response) => {
    ...
    bot.on('messageCreate', message => {
        ...
        switch (args[0].toLowerCase()) {
            // Below is the command. This resets the bot on the Owner's consent.
            case 'reset':
                if (!isBotOwner) {
            ...
            case 'help':
                message.reply('Type &mcstats <IP> <Server Port> to get the stats on the server you are trying to use.\n Only the server owner can reset the bot and use &shutdown and &reset .');
                return;
